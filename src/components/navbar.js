...
//   const { userInfo } = useSelector(({ auth }) => auth.authUser)
  ...
  const openRoutes = ['/', '/about', '/faq', '/contact-us', '/login', '/sign-up', '/how-it-works']
    ...
    <nav className='nav-bar border-b border-gray-100'>
      <Link to='/'>
        <div className='flex items-center flex-shrink-0 text-blacker mr-6'>
          <h1 className='font-manrope font-black text-primary text-4xl ml-20'>Ajot</h1>
        </div>
      </Link>
      ...
      >
        <div className='lg:flex-grow lg:text-right text-base'>
          <Link
            to='/'
            className={`${
              pathname === '/' ? 'border-b-2  font-bold text-primary' : 'text-black'
        ...
        {!userInfo ? (
          <div>
            <Link to='/login' className='nav-btn rounded-md outline-none hover:text-white text-primary font-manrope font-medium border border-primary'>
              Login
            </Link>
            <Link to='/sign-up' className='nav-btn rounded-md hover:text-gray-300 text-white bg-primary'>
              Sign Up
            </Link>
          </div>
        ) : (
          <Link to='/dashboard' className='nav-btn hover:text-gray-300 text-white bg-primary'>
            Dashboard
          </Link>
        )}
      </div>
